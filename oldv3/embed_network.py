...
class TextClassifierTrainer(MRJob):
    ...
    def combiner(self, key, records):
        ...
        if len(batch_X) > 0:
            key = 1
            if self.mapper_split != 0:
                key = len(batch_X) % self.mapper_split
    ...
    def reducer(self, key, grads):
        ...
        total_fc1_weight_grads = np.zeros_like(self.theta1)
        total_fc2_weight_grads = np.zeros_like(self.theta2)
        total_fc1_bias_grads = np.zeros_like(self.bias1)
        total_fc2_bias_grads = np.zeros_like(self.bias2)
        ...
        for grad in grads:
            ...
            count += 1
        if (count > 0):
            self.theta1 -= self.fc1_lr / count * total_fc1_weight_grads
            self.theta2 -= self.fc2_lr / count * total_fc2_weight_grads
            self.bias1 -= self.fc1_lr / count * total_fc1_bias_grads
            self.bias2 -= self.fc2_lr / count * total_fc2_bias_grads
            self.embed -= self.embed_lr / count * total_embedding_grads
            self.log = {
                "fc1" : total_fc1_weight_grads.tolist(),
                "fc2" : total_fc2_weight_grads.tolist(),
                "embed" : total_embedding_grads.tolist()
            }
            yield None, {
                "Status" : "Success"
            }
