...
class TextClassifierTrainer(MRJob):
    ...
    def __init__(self, *args, **kwargs):
        ...
        from util import get_vocab, EMBED_DIM, SEQ_LENGTH, HIDDEN_SIZE, OUTPUT_SIZE, MAPPER_SPLIT, NN_LR
        vocab = get_vocab()
    ...
    def load_weight(self):
        ...
        if ("theta1" in WEIGHT):
            ...
            self.theta1 = np.zeros((self.hidden_layer_size, self.input_layer_size + 1))
            self.theta2 = np.zeros((self.output_layer_size, self.hidden_layer_size + 1))
        if ("embed" in WEIGHT):
            self.embed = WEIGHT["embed"]
        else:
            raise Exception("No embed found")
